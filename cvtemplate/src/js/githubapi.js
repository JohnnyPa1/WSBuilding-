...
function getData(gitHubUser) {
    let rezultat = new Promise(function (resolve, reject) {
        $.ajax({
            ...
            url: `https://api.github.com/users/${gitHubUser}/repos?per_page=6&page=1&sort=`,
...
function showData(repos) {
    ...
    repos.forEach(item => {
        if(item.language == null){
            ...
            var itemLanguage = `<span>Programming Language: ${item.language}</span>`;
        }
        allRepo += `
        <div class="col-md-4">
...
$(function () {
    console.log("ready!");
    getData('JohnnyPa1').then((myRepos) => {
        showData(myRepos);
